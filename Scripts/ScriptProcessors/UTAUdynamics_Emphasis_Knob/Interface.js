...
const var inputPanel = Content.addPanel("pnlInput");
const var outputPanel = Content.addPanel("pnlOutput");
...
inputPanel.setPaintRoutine(function(g)
{
    ...
    var fade = Content.getComponent("knbFade").getValue();
    var slackLevel = floor + slack;
    var fadeInLevel = floor + fade * 9;
    var fadeOutLevel = slackLevel - fade * 9;
    
    var rawLevel = PeakIn.getCurrentLevel(1);
    var LeveldB = (rawLevel > 0) ? Engine.getDecibelsForGainFactor(rawLevel) : -60.0;
    
    var peak = dbToY(LeveldB, h);
    var maxPeakY = dbToY(this.data.maxPeakdB, h);
    var floorY = dbToY(floor, h);
    var slackY = dbToY(slackLevel, h);
    var fadeInY = dbToY(fadeInLevel, h);
    var fadeOutY = dbToY(fadeOutLevel, h);
...
outputPanel.setPaintRoutine(function(g)
{
	...
	var rawLevel = PeakOut.getCurrentLevel(1);
	var LeveldB = (rawLevel > 0) ? Engine.getDecibelsForGainFactor(rawLevel) : -60.0;
...
const var knobConfig = [
    ...
    { id: "knbColor",  module: Dynamics, index: 2, title: "COLR", unit: "" },
...
{
    var cfg = findConfig(component.get("id"));
